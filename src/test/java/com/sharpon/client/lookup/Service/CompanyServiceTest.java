...
class CompanyServiceTest {
    ...
    void findById_withValidUserId_findsSuccessfully() {
        ...
        CompanyDto actual = service.findById(1L);
    ...
    void save_withValidInput_savesSuccessfully() {
        ...
        when(mapper.toDto(entity)).thenReturn(dto);
        when(repository.save(any(Company.class))).thenReturn(entity);
        CompanyDto response = service.save(dto);
    ...
    void search_withValidInput_shouldReturnList() {
        Page<Company> expected = new PageImpl<>(Collections.singletonList(entity));

        when(mapper.toDto(any(Company.class))).thenReturn(dto);
        when(repository.findAll(any(Pageable.class))).thenReturn(expected);

        Page<CompanyDto> response = service.search(Pageable.unpaged());
