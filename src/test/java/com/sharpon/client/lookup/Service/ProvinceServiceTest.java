...
class ProvinceServiceTest {
    ...
    void findById_withValidUserId_findsSuccessfully() {
        ...
        ProvinceDto actual = service.findById(1L);
    ...
    void delete_withValidUserId_deletesSuccessfully() {
        when(repository.findById(anyLong())).thenReturn(Optional.of(entity));
        doNothing().when(repository).delete(any(Province.class));
    ...
    void save_withValidInput_savesSuccessfully() {
        ...
        when(mapper.toDto(entity)).thenReturn(dto);
        when(repository.save(any(Province.class))).thenReturn(entity);
        ProvinceDto response = service.save(dto);

        assertEquals(dto, response);
        verify(repository, times(1)).save(any(Province.class));
    }
    ...
    void search_withValidInput_shouldReturnList() {
        Page<Province> expected = new PageImpl<>(Collections.singletonList(entity));

        when(mapper.toDto(any(Province.class))).thenReturn(dto);
        when(repository.findAll(any(Pageable.class))).thenReturn(expected);

        Page<ProvinceDto> response = service.search(Pageable.unpaged());
