...
class CityServiceTest {
    ...
    void findById_withValidUserId_findsSuccessfully() {
        ...
        CityDto actual = service.findById(1L);
    ...
    void delete_withValidUserId_deletesSuccessfully() {
        when(repository.findById(anyLong())).thenReturn(Optional.of(entity));
        doNothing().when(repository).delete(any(City.class));
    ...
    void save_withValidInput_savesSuccessfully() {
        ...
        when(mapper.toDto(entity)).thenReturn(dto);
        when(repository.save(any(City.class))).thenReturn(entity);
        CityDto response = service.save(dto);
    ...
    void search_withValidInput_shouldReturnList() {
        Page<City> expected = new PageImpl<>(Collections.singletonList(entity));

        when(mapper.toDto(any(City.class))).thenReturn(dto);
        when(repository.findAll(any(Pageable.class))).thenReturn(expected);

        Page<CityDto> response = service.search(Pageable.unpaged());
