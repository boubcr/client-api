...
class CountryServiceTest {
    ...
    void findById_withValidUserId_findsSuccessfully() {
        ...
        CountryDto actual = service.findById(1L);
    ...
    void delete_withValidUserId_deletesSuccessfully() {
        when(repository.findById(anyLong())).thenReturn(Optional.of(entity));
        doNothing().when(repository).delete(any(Country.class));
    ...
    void save_withValidInput_savesSuccessfully() {
        ...
        CountryDto response = service.save(dto);
    ...
    void search_withValidInput_shouldReturnList() {
        Page<Country> expected = new PageImpl<>(Collections.singletonList(entity));

        when(mapper.toDto(any(Country.class))).thenReturn(dto);
        when(repository.findAll(any(Pageable.class))).thenReturn(expected);

        Page<CountryDto> response = service.search(Pageable.unpaged());
