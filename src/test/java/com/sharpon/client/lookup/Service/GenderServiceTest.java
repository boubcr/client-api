...
class GenderServiceTest {
    ...
    void findById_withValidUserId_findsSuccessfully() {
        ...
        GenderDto actual = service.findById(1L);
    ...
    void delete_withValidUserId_deletesSuccessfully() {
        when(repository.findById(anyLong())).thenReturn(Optional.of(entity));
        doNothing().when(repository).delete(any(Gender.class));
    ...
    void save_withValidInput_savesSuccessfully() {
        ...
        when(mapper.toDto(entity)).thenReturn(dto);
        when(repository.save(any(Gender.class))).thenReturn(entity);
        GenderDto response = service.save(dto);

        assertEquals(dto, response);
        verify(repository, times(1)).save(any(Gender.class));
    }
    ...
    void search_withValidInput_shouldReturnList() {
        Page<Gender> expected = new PageImpl<>(Collections.singletonList(entity));

        when(mapper.toDto(any(Gender.class))).thenReturn(dto);
        when(repository.findAll(any(Pageable.class))).thenReturn(expected);

        Page<GenderDto> response = service.search(Pageable.unpaged());
